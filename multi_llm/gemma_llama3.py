...
model2_id = "meta-llama/Meta-Llama-3-8B-Instruct"
    ...
    with jsonlines.open(args.output_path, mode="w") as outfile:
        with jsonlines.open(args.input_path) as file:
            for line in file.iter():
                ...
                input_ids = tokenizer2.apply_chat_template(messages, add_generation_prompt=True, return_tensors="pt").to(model2.device)
                terminators = [tokenizer2.eos_token_id, tokenizer2.convert_tokens_to_ids("<|eot_id|>")]
                outputs = model2.generate(input_ids, max_new_tokens=1024, eos_token_id=terminators, do_sample=False, temperature=0.0,)
                response = outputs[0][input_ids.shape[-1]:]
                generation2_1 = tokenizer2.decode(response, skip_special_tokens=True)
                ...
                outputs = model2.generate(input_ids, max_new_tokens=1024, eos_token_id=terminators, do_sample=False, temperature=0.0,)
                response = outputs[0][input_ids.shape[-1]:]
                generation2_2 = tokenizer2.decode(response, skip_special_tokens=True)
                ...
                outputs = model2.generate(input_ids, max_new_tokens=1024, eos_token_id=terminators, do_sample=False, temperature=0.0,)
                response = outputs[0][input_ids.shape[-1]:]
                generation2_3 = tokenizer2.decode(response, skip_special_tokens=True)
