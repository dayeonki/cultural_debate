...
model2_id = "internlm/internlm2_5-7b"
...
def main():
    ...
    tokenizer2 = AutoTokenizer.from_pretrained(model2_id, cache_dir=own_cache_dir, trust_remote_code=True)
    model2 = AutoModelForCausalLM.from_pretrained(model2_id, cache_dir=own_cache_dir, torch_dtype=torch.float16, trust_remote_code=True).cuda()
    model2 = model2.eval()
    ...
    with jsonlines.open(args.output_path, mode="w") as outfile:
        with jsonlines.open(args.input_path) as file:
            for line in file.iter():
                ...
                print(prompt2_1)
                inputs = tokenizer2([prompt2_1], return_tensors="pt")
                for k,v in inputs.items():
                    inputs[k] = v.cuda()
                gen_kwargs = {"max_length": 1024, "temperature": 0.0, "do_sample": False}
                output = model2.generate(**inputs, **gen_kwargs)
                generation2_1 = tokenizer2.decode(output[0].tolist(), skip_special_tokens=True)
                ...
                print(prompt2_2)
                inputs = tokenizer2([prompt2_2], return_tensors="pt")
                for k,v in inputs.items():
                    inputs[k] = v.cuda()
                gen_kwargs = {"max_length": 1024, "temperature": 0.0, "do_sample": False}
                output = model2.generate(**inputs, **gen_kwargs)
                generation2_2 = tokenizer2.decode(output[0].tolist(), skip_special_tokens=True)
                ...
                print(prompt2_3)
                inputs = tokenizer2([prompt2_3], return_tensors="pt")
                for k,v in inputs.items():
                    inputs[k] = v.cuda()
                gen_kwargs = {"max_length": 1024, "temperature": 0.0, "do_sample": False}
                output = model2.generate(**inputs, **gen_kwargs)
                generation2_3 = tokenizer2.decode(output[0].tolist(), skip_special_tokens=True)
                ...
                line[f"internlm_1"] = generation2_1
                line[f"gemma_2"] = generation1_2
                line[f"internlm_2"] = generation2_2
                line[f"gemma_final"] = generation1_3
                line[f"internlm_final"] = generation2_3
                outfile.write(line)
