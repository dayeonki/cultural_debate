    ...
    with jsonlines.open(args.output_path, mode="w") as outfile:
        with jsonlines.open(args.input_path) as file:
            for line in file.iter():
                ...
                input_ids = tokenizer1.apply_chat_template(conversation=messages, tokenize=True, return_tensors='pt')
                output_ids = model1.generate(input_ids.to('cuda'), eos_token_id=tokenizer1.eos_token_id, max_length=1024, temperature=0.0)
                generation1_1 = tokenizer1.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
                ...
                input_ids = tokenizer1.apply_chat_template(conversation=messages, tokenize=True, return_tensors='pt')
                output_ids = model1.generate(input_ids.to('cuda'), eos_token_id=tokenizer1.eos_token_id, max_length=1024, temperature=0.0)
                generation1_2 = tokenizer1.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
                ...
                input_ids = tokenizer1.apply_chat_template(conversation=messages, tokenize=True, return_tensors='pt')
                output_ids = model1.generate(input_ids.to('cuda'), eos_token_id=tokenizer1.eos_token_id, max_length=1024, temperature=0.0)
                generation1_3 = tokenizer1.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
