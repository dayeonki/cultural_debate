    ...
    with jsonlines.open(args.output_path, mode="w") as outfile:
        with jsonlines.open(args.input_path) as file:
            for line in file.iter():
                ...
                output_ids = model2.generate(input_ids.to('cuda'), eos_token_id=tokenizer2.eos_token_id, max_length=1024)
                generation2_1 = tokenizer2.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
                ...
                input_ids = tokenizer2.apply_chat_template(conversation=messages, tokenize=True, return_tensors='pt')
                output_ids = model2.generate(input_ids.to('cuda'), eos_token_id=tokenizer2.eos_token_id, max_length=1024)
                generation2_2 = tokenizer2.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
                ...
                input_ids = tokenizer2.apply_chat_template(conversation=messages, tokenize=True, return_tensors='pt')
                output_ids = model2.generate(input_ids.to('cuda'), eos_token_id=tokenizer2.eos_token_id, max_length=1024)
                generation2_3 = tokenizer2.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
